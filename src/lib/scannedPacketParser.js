...
function classifyByText(text) {
  ...
  if (/\border\b/i.test(value) && /order\s*no/i.test(value)) return { type: 'bistrack_order', label: 'BisTrack Order', confidence: 'high' }
  if (/invoice/i.test(value) && /(invoice\s*no|invoice\s*total)/i.test(value)) return { type: 'bistrack_invoice', label: 'BisTrack Invoice', confidence: 'high' }
  if (/receipt/i.test(value) && /(receipt\s*no|amount\s*paid)/i.test(value)) return { type: 'bistrack_receipt', label: 'BisTrack Receipt', confidence: 'high' }
...
export function buildScannedPacket(ocrPages) {
  const pages = ocrPages.map((page) => {
    ...
    return {
      ...
      status: classification.type.startsWith('bistrack') ? 'Needs Review' : 'Reference',
    }
  })

  const followUpItems = pages
    .filter((page) => page.classification.type === 'bistrack_quote')
    .map((page) => ({
      pageNumber: page.pageNumber,
